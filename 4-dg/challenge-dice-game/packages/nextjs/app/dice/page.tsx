...
interface RiggedRollAttempt {
  predictedRoll: bigint;
  called: boolean;
}
...
const DiceGame: NextPage = () => {
  ...
  const [riggedAttempts, setRiggedAttempts] = useState<RiggedRollAttempt[]>([]);
  ...
  // Listen for RiggedRollAttempt events
  const { data: riggedAttemptData, isLoading: riggedAttemptLoading } = useScaffoldEventHistory({
    contractName: "RiggedRoll",
    eventName: "RiggedRollAttempt",
    watch: true,
  });

  useEffect(() => {
    if (
      !riggedAttemptLoading &&
      Boolean(riggedAttemptData?.length) &&
      (riggedAttemptData?.length as number) > riggedAttempts.length
    ) {
      // Stop rolling animation when RiggedRollAttempt event is received
      setIsRolling(false);

      setRiggedAttempts(
        riggedAttemptData?.map(({ args }) => ({
          predictedRoll: args.predictedRoll as bigint,
          called: args.called as boolean,
        })) || [],
      );
    }
  }, [riggedAttemptData, riggedAttemptLoading, riggedAttempts.length]);
      ...
      <div className="grid grid-cols-3 max-lg:grid-cols-1">
        ...
        <div className="flex flex-col items-center pt-4 max-lg:row-start-1">
          ...
          <div className="mt-4 pt-2 flex flex-col items-center w-full justify-center border-t-4 border-primary">
            ...
            {/* Show last rigged attempt result */}
            {riggedAttempts.length > 0 && (
              <div className="mt-2 text-sm">
                Last prediction: {riggedAttempts[0].predictedRoll.toString()} 
                {riggedAttempts[0].called ? " (Called rollTheDice!)" : " (Skipped - would lose)"}
              </div>
            )}
          </div>
